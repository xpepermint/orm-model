...
var infl = new (require('natural').NounInflector)();
  ...
  var loadModels = function(discriminator) {
    fs.readdirSync(rootPath).forEach(function(fname) {
      ...
      if (fs.statSync(fpath).isFile()) {
        ...
        if (connections.object(data.connector).orm == 'mongoose') {
          // load discriminator
          if (discriminator && data.model) {
            loadDiscriminator(name, data);
          }
          // load model
          else if (!discriminator && !data.model) {
            loadModel(name, data);
          }
        }
      }
